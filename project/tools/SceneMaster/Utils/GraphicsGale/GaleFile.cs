...
namespace GraphicsGaleWrapper
{
    ...
    public class GaleObject:IDisposable
    {
        ...
        public ColorPalette Palette {
            get {
                ...
                if(palette == null) {
                    using(var bmp = GaleFile.GetBitmap(Handle, 0, 0)) {
                        palette = bmp.Palette;
                    }
                }
                return palette;
        ...
        public GaleObject(string path) {
            Handle = GaleFile.Open(path);

            if(Handle.ToInt64() == 0) {
                switch(GaleFile.GetLastError()) {
            ...
            Width = GaleFile.GetWidth(Handle);
            Height = GaleFile.GetHeight(Handle);

            SinglePalette = GaleFile.IsSinglePalette(Handle);
            BackgroundColor = GaleFile.GetBackgroundColor(Handle);

            DisabledBottomLayerTransparency = GaleFile.IsBottomLayerTransparencyDisabled(Handle);

            BitsPerPixel = GaleFile.GetBitsPerPixel(Handle);
            IndexedColor = BitsPerPixel <= 8;

            FrameCount = GaleFile.GetFrameCount(Handle);
            _ZeroDigitFormat = new String('0', FrameCount.ToString().Length);

        }

        public void Dispose() {
            GaleFile.Close(Handle);
            frames = null;
            Handle = IntPtr.Zero;
        }
    }
    ...
    public class Frame
    {
        ...
        public ColorPalette Palette {
            get {
                ...
                if(palette == null) {
                    using(var bmp = GaleFile.GetBitmap(galeObject.Handle, frameIndex, 0)) {
                        palette = bmp.Palette;
                    }
                }
                return palette;
        ...
        internal Frame(GaleObject galeObject, int frameIndex) {
            ...
            InternalName = GaleFile.GetFrameName(galeObject.Handle, frameIndex);
            ...
            TransparentColor = GaleFile.GetFrameTransparentColor(galeObject.Handle, this.frameIndex);
            DisposalType = GaleFile.GetFrameDisposalType(galeObject.Handle, this.frameIndex);
            Delay = GaleFile.GetFrameDelay(galeObject.Handle, this.frameIndex);

            LayerCount = GaleFile.GetLayerCount(galeObject.Handle, frameIndex);
        }

        public Bitmap CreateBitmap() {
            var bmp = GaleFile.GetBitmap(galeObject.Handle, frameIndex, GaleFile.ALL_LAYERS);
    ...
    public class Layer
    {
        ...
        internal Layer(GaleObject galeObject, int frameIndex, int layerIndex) {
            ...
            Name = GaleFile.GetLayerName(galeObject.Handle, frameIndex, layerIndex);

            Visible = GaleFile.IsLayerVisible(galeObject.Handle, frameIndex, layerIndex);
            AlphaChannelEnabled = GaleFile.IsAlphaChannelEnabled(galeObject.Handle, frameIndex, layerIndex);

            TransparentColor = GaleFile.GetLayerTransparentColor(galeObject.Handle, frameIndex, layerIndex);
            Opacity = GaleFile.GetLayerOpacityLevel(galeObject.Handle, frameIndex, layerIndex);
        }

        public Bitmap CreateBitmap() {
            var bmp = GaleFile.GetBitmap(galeObject.Handle, frameIndex, layerIndex);
