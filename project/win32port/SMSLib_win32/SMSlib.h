...
#define VOLATILE_AT(x)
#define AT(x)
#define SFR_AT(x)
#else
#define VOLATILE_AT(x) volatile __at (x) 
#define AT(x) __at (x)
#define SFR_AT(x) __sfr __at 
#endif
...
VOLATILE_AT(0xffff) unsigned char ROM_bank_to_be_mapped_on_slot2;
...
VOLATILE_AT(0xfffe) unsigned char ROM_bank_to_be_mapped_on_slot1;
VOLATILE_AT(0xfffd) unsigned char ROM_bank_to_be_mapped_on_slot0;

/* macro for SRAM access */
VOLATILE_AT(0xfffc) unsigned char SRAM_bank_to_be_mapped_on_slot2;
...
AT (0x8000) unsigned char SMS_SRAM[];
...
void SMS_addSprite_noreturn_f (unsigned int y, unsigned int x_tile) __naked __preserves_regs(d,e,iyh,iyl) __sdcccall(1);        /* doesn't return anything */
...
void SMS_addMetaSprite_f (unsigned int origin_yx, const void *metasprite) __naked __sdcccall(1);                                     /* doesn't return anything */
...
SFR_AT(0xbf) SMS_VDPControlPort;
...
#define SMS_EMBED_SEGA_ROM_HEADER(productCode,revision)                                        \
 const AT (0x7ff0) unsigned char __SMS__SEGA_signature[16]={'T','M','R',' ','S','E','G','A', \
                                                                          0xFF,0xFF,0xFF,0xFF, \
...
#define SMS_EMBED_SDSC_HEADER(verMaj,verMin,dateYear,dateMonth,dateDay,author,name,descr)      \
                          const AT (0x7fe0-sizeof(author)) char __SMS__SDSC_author[]=author; \
                 const AT (0x7fe0-sizeof(author)-sizeof(name)) char __SMS__SDSC_name[]=name; \
 const AT (0x7fe0-sizeof(author)-sizeof(name)-sizeof(descr)) char __SMS__SDSC_descr[]=descr; \
                          const AT (0x7fe0) char __SMS__SDSC_signature[16]={'S','D','S','C', \
...
#define SMS_EMBED_SEGA_ROM_HEADER_16KB(productCode,revision)                                   \
 const AT (0x3ff0) unsigned char __SMS__SEGA_signature[16]={'T','M','R',' ','S','E','G','A', \
                                                                          0xFF,0xFF,0xFF,0xFF, \
                  SMS_BYTE_TO_BCD((productCode)%100),SMS_BYTE_TO_BCD(((productCode)/100)%100), \
...
#define SMS_EMBED_SDSC_HEADER_16KB(verMaj,verMin,dateYear,dateMonth,dateDay,author,name,descr) \
                          const AT (0x3fe0-sizeof(author)) char __SMS__SDSC_author[]=author; \
                 const AT (0x3fe0-sizeof(author)-sizeof(name)) char __SMS__SDSC_name[]=name; \
 const AT (0x3fe0-sizeof(author)-sizeof(name)-sizeof(descr)) char __SMS__SDSC_descr[]=descr; \
                          const AT (0x3fe0) char __SMS__SDSC_signature[16]={'S','D','S','C', \
                                              SMS_BYTE_TO_BCD(verMaj),SMS_BYTE_TO_BCD(verMin), \
                                          SMS_BYTE_TO_BCD(dateDay),SMS_BYTE_TO_BCD(dateMonth), \
                              SMS_BYTE_TO_BCD((dateYear)%100),SMS_BYTE_TO_BCD((dateYear)/100), \
